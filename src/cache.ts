...
import { factCheck, type Logger } from "./fact-check";
...
export async function createResultCacheKey(input: FactCheckInput, origin: string): Promise<Request> {
  // 只共用經 parseInput 正規化後完全相同的輸入；模型、限制與提示變動會自動產生新鍵。
  ...
  // 鍵只含雜湊，不將原文、URL 或 credential 放進 Cache API 的 request URL。
...
function parseCacheEntry(value: unknown): CacheEntry {
  ...
  // related_checks 只列 Cofacts 回覆；白名單機構網址也可使 no_relevant_evidence 為 false。
  if (
  ...
  // parseSynthesis 會回傳下修後的副本；快取原值超過上限時整筆拒絕。
  ...
  for (const value of checks) {
    ...
    if (check.type !== "cofacts_human" && check.type !== "cofacts_ai") {
      throw new Error("快取類型不正確。");
    }
    ...
    if (check.reference_urls !== undefined) {
      array(check.reference_urls).forEach((url) => parseText(url, LIMITS.url));
    }
    if (
...
export async function cachedFactCheck(
  ...
  // Worker 環境交給 waitUntil；無 execution context 的測試環境同步等待寫入。
